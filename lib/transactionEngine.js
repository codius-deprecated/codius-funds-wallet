...
function processPayment(payment, callback) {
	User.get(payment.source, function(error, sender){
		...
		User.get(payment.destination, function(error, receiver){
			...
			if (!sender) {
				return callback(new Error('Cannot execute payment. User: ' + payment.source + ' does not exist.'));
			}
			if (!receiver) {
				return callback(new Error('Cannot execute payment. User: ' + payment.destination + ' does not exist.'));
			}
			...
			if (senderBalanceIndex === -1 || sender.balances[senderBalanceIndex].balance < payment.amount) {
				return callback(new Error('Insufficient funds to execute payment.'));
			}
